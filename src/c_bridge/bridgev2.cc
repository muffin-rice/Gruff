...
Bridgev2Game::BridgeGame(const GameParameters& params)
    : Game(kGameType, params) {}

Bridgev2State::BridgeState(std::shared_ptr<const Game> game,
...
std::string Bridgev2State::ActionToString(Player player, Action action) const {
  return (action < kBiddingActionBase) ? CardString(action)
                                       : BidString(action - kBiddingActionBase);
}

std::string Bridgev2State::ToString() const {
...
std::string Bridgev2State::ObservationString(Player player) const {
...
std::array<absl::optional<Player>, kNumCards> Bridgev2State::OriginalDeal()
    const {
...
std::string Bridgev2State::FormatDeal() const {
...
std::string Bridgev2State::FormatVulnerability() const {
...
std::string Bridgev2State::FormatAuction(bool trailing_query) const {
...
std::string Bridgev2State::FormatPlay() const {
...
std::string Bridgev2State::FormatResult() const {
...
void Bridgev2State::ObservationTensor(Player player,
                                    absl::Span<float> values) const {
  SPIEL_CHECK_EQ(values.size(), game_->ObservationTensorSize());
  WriteObservationTensor(player, values);
}

void Bridgev2State::WriteObservationTensor(Player player,
                                         absl::Span<float> values) const {
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
...
std::vector<double> Bridgev2State::PublicObservationTensor() const {
...
std::vector<double> Bridgev2State::PrivateObservationTensor(Player player) const {
...
void Bridgev2State::SetDoubleDummyResults(ddTableResults double_dummy_results) {
...
void Bridgev2State::ComputeDoubleDummyTricks() const {
...
std::vector<int> Bridgev2State::ScoreForContracts(
...
std::vector<Action> Bridgev2State::LegalActions() const {
  switch (phase_) {
    case Phase::kDeal:
...
std::vector<Action> Bridgev2State::DealLegalActions() const {
...
std::vector<Action> Bridgev2State::BiddingLegalActions() const {
  std::vector<Action> legal_actions;
...
std::vector<Action> Bridgev2State::PlayLegalActions() const {
  std::vector<Action> legal_actions;
...
std::vector<std::pair<Action, double>> Bridgev2State::ChanceOutcomes() const {
...
void Bridgev2State::DoApplyAction(Action action) {
  switch (phase_) {
    case Phase::kDeal:
...
void Bridgev2State::ApplyDealAction(int card) {
...
void Bridgev2State::ApplyBiddingAction(int call) {
...
void Bridgev2State::ApplyPlayAction(int card) {
...
Player Bridgev2State::CurrentPlayer() const {
...
void Bridgev2State::ComputeScoreByContract() const {
  SPIEL_CHECK_TRUE(double_dummy_results_.has_value());
...
// results.
std::string Bridgev2State::Serialize() const {
...
int Bridgev2State::ContractIndex() const {
